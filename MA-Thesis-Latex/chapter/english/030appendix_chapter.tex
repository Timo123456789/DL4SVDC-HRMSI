...
\begin{figure}[htbp]
    ...
    \caption[Comparison of \acrshort{mAP}@0.5--0.95 values for \acrshort{RGBIR}, \acrshort{RGB}, \acrshort{RGIR} on different YOLO versions pretrained and \textit{from scratch} (Best validation model on validation dataset)]{Comparison of \acrshort{mAP}@0.5--0.95 values for \acrshort{RGBIR}, \acrshort{RGB}, \acrshort{RGIR} on different \acrshort{YOLO} versions pretrained and \textit{from scratch}. (Best validation model on validation dataset). It turns out that all three spectral permutations consistently performed worse in scratch training than pre-trained models. There were hardly any differences between \acrshort{RGBIR} and \acrshort{RGIR} in both \acrshort{YOLO} versions. However, in \acrshort{RGB}, \acrshort{YOLO}v12 performed significantly worse than \acrshort{YOLO}v9 \textit{from scratch}, accompanied by the largest quantile range. These results suggest that the choice of training strategy has a stronger influence on model performance than the \acrshort{YOLO} variant used.  (Own representation).}
