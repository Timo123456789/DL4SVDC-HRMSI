...
Further experiments were also conducted based on the \acrshort{DOTA} dataset to evaluate the performance of different \acrshort{YOLO} versions. \acrshort{YOLO}v3 (released in \citeyear{yolov3_redmon} \cite{yolov3_redmon}), \acrshort{YOLO}v9 (\citeyear{wang2024} \cite{wang2024}) and \acrshort{YOLO}v12 (\citeyear{yolo12} \cite{tian2025}, \cite{yolo12}) were used, each for 500 epochs under otherwise identical hyperparameters according to Tab.~\ref{tab:hyperparameters}. \acrshort{YOLO}v3 (in the Ultralytics implementation, referred to as \acrshort{YOLO}v3u, which does not include the typical \acrshort{YOLO} detection head) is the oldest variant, while \acrshort{YOLO}v12 represents the standard Ultralytics architecture from 2025; the architectural changes are described in section \ref{sec:milestone_yolo}. The models were evaluated using the \acrshort{mAP}@0.5--0.95, with \acrshort{YOLO}v3 achieving an average value of 0.34 (see Figure \ref{fig:yolov3_dota}). In comparison, \acrshort{YOLO}v12 showed a significantly improved performance of 0.40 (\ref{fig:yolov12_dota}), while \acrshort{YOLO}v9 achieved the best value of 0.42 (see fig.~\ref{fig:DOTA_train_values}).
