...
Further experiments were also conducted based on the \acrshort{DOTA} dataset to evaluate the performance of different \acrshort{YOLO} versions. \acrshort{YOLO}v3, \acrshort{YOLO}v9 and \acrshort{YOLO}v12 were used, each for 500 epochs under otherwise identical hyperparameters according to Tab. \ref{tab:hyperparameters}. \acrshort{YOLO}v3 is the oldest variant, while \acrshort{YOLO}v12 represents a newer architecture from 2024; the architectural changes are described in section \ref{sec:milestone_yolo}. The models were evaluated using the \acrshort{mAP}@0.5--0.95, with \acrshort{YOLO}v3 achieving an average value of 0.34 (see Figure \ref{fig:yolov3_dota}). In comparison, \acrshort{YOLO}v12 showed a significantly improved performance of 0.40 (\ref{fig:yolov12_dota}), while \acrshort{YOLO}v9 achieved the best value of 0.42 (see Figure \ref{fig:DOTA_train_values}).
...
To investigate the influence of scratch training, i.e., training \textit{from scratch}, compared to the use of pre-trained models, YOLOv9 and YOLOv12 were trained under identical hyperparameters (Table \ref{tab:hyperparameters}) with the spectral permutations RGBIR, RGB, and RGIR. All three permutations consistently performed worse than the pre-trained models when trained \textit{from scratch} (see Figs. \ref{fig:add_exp_map50-95:val_on_test} and \ref{fig:add_exp_map50-95:val_on_val}). For both YOLO versions, only minimal differences were observed between RGBIR and RGIR. However, for the RGB permutation, YOLOv12, which was trained \textit{from scratch}, performed significantly worse than YOLOv9 and also had the largest quantile range. %These results show that both the choice of training strategy and the YOLO version have a significant impact on model performance.
